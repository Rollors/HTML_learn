...
*   函数 --一段预定义好、并可以被反复使用的代码块
    ...
    *   函数的调用 --直接使用函数名
    *   定义函数的返回值 --return
*   DOM --万维网联盟的标准，允许程序和脚本动态访问和更新文档的内容、结构和样式 _专用于操作文档的API_
...
*   BOM 
    *   概述 --Browser Object Model _专用操作浏览器窗口的API_:没有标准、有兼容性问题
    *   浏览器对象模型
...
*   正则表达式 --由普通字符和元字符组成
    *   元字符
        *   . --匹配除换行符以外的任意一个字符
        *   \w --匹配任意一个字母、数字以及下划线，等价于[0-9a-zA-Z]
        *   \W --匹配除字母、数字、下划线以及汉字以外的任意一个字符，等价于[^0-9a-zA-Z]
        *   \s --匹配任意一个空白符 等价于[\f\n\r\t\v]
        *   \S --匹配任意一个非空白符，等价于[^\f\n\r\t\v]
